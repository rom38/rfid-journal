class RFIDBluetoothMonitor {
    constructor() {
        ...
        // Все возможные UUID сервисов для поиска
        this.OPTIONAL_SERVICES = [
            // 16-битные UUID в 128-битном формате
            '00004faf-0000-1000-8000-00805f9b34fb',
            '0000beb5-0000-1000-8000-00805f9b34fb',
            // Стандартные сервисы BLE
    ...
    async connect() {
        try {
            ...
            // Используем acceptAllDevices и запрашиваем все возможные сервисы
            ...
            for (let service of services) {
                ...
                try {
                    ...
                    for (let char of characteristics) {
                        ...
                        this.addLog(`    Свойства: read=${char.properties.read}, notify=${char.properties.notify}, write=${char.properties.write}, indicate=${char.properties.indicate}`, 'info');
                    }
                } catch (error) {
            ...
            const result = await this.findSuitableServiceAndCharacteristic(services);
            if (!result) {
            ...
            this.addLog(`Выбран сервис: ${this.service.uuid}`, 'success');
            this.addLog(`Выбрана характеристика: ${this.characteristic.uuid}`, 'success');
    ...
    async findSuitableServiceAndCharacteristic(services) {
        // Приоритет 1: сервисы с характеристиками, поддерживающими уведомления
        for (let service of services) {
            try {
                const characteristics = await service.getCharacteristics();
                
                // Ищем характеристику с уведомлениями или индикациями
        ...
        // Приоритет 2: сервисы с характеристиками, поддерживающими чтение
        for (let service of services) {
            try {
                const characteristics = await service.getCharacteristics();
        ...
        // Приоритет 3: любой сервис с любой характеристикой
        for (let service of services) {
            try {
                const characteristics = await service.getCharacteristics();
