class RFIDBluetoothMonitor {
    constructor() {
        ...
        // UUID из вашего Arduino кода (16-битные в 128-битном формате)
        this.SERVICE_UUID = '00004faf-0000-1000-8000-00805f9b34fb';
        this.CHARACTERISTIC_UUID = '0000beb5-0000-1000-8000-00805f9b34fb';
        
        // Дополнительные сервисы для совместимости
    ...
    async connect() {
        try {
            ...
            // Запрашиваем устройство с указанием сервисов
            ...
            // Пытаемся подключиться к нашему сервису
            try {
                this.service = await this.server.getPrimaryService(this.SERVICE_UUID);
                this.addLog(`Найден сервис RFID: ${this.SERVICE_UUID}`, 'success');
            } catch (error) {
                this.addLog(`Не удалось найти сервис RFID: ${error.message}`, 'warning');
                // Если не нашли наш сервис, ищем альтернативные
                this.service = await this.findAlternativeService();
            }
            ...
            // Пытаемся найти нашу характеристику
            try {
                this.characteristic = await this.service.getCharacteristic(this.CHARACTERISTIC_UUID);
                this.addLog(`Найдена характеристика RFID: ${this.CHARACTERISTIC_UUID}`, 'success');
            } catch (error) {
                this.addLog(`Не удалось найти характеристику RFID: ${error.message}`, 'warning');
                this.characteristic = await this.findAlternativeCharacteristic();
            }

            if (!this.characteristic) {
                throw new Error('Не найдена подходящая характеристика');
            }
            ...
            if (this.characteristic.properties.notify) {
                ...
                throw new Error('Характеристика не поддерживает уведомления или чтение');
            }
    ...
    async findAlternativeService() {
        try {
            const services = await this.server.getPrimaryServices();
            this.addLog(`Доступно сервисов: ${services.length}`, 'info');
            
            // Логируем все доступные сервисы
            services.forEach(service => {
                this.addLog(`Доступный сервис: ${service.uuid}`, 'info');
            });

            // Ищем сервисы с характеристиками
            for (let service of services) {
                try {
                    const characteristics = await service.getCharacteristics();
                    if (characteristics.length > 0) {
                        this.addLog(`Выбран альтернативный сервис: ${service.uuid}`, 'success');
                        return service;
                    }
                } catch (error) {
                    continue;
            ...
            this.addLog(`Ошибка поиска альтернативных сервисов: ${error.message}`, 'error');
            return null;
        }
    }

    async findAlternativeCharacteristic() {
        try {
            const characteristics = await this.service.getCharacteristics();
            this.addLog(`Доступно характеристик: ${characteristics.length}`, 'info');
            
            // Логируем все характеристики
            characteristics.forEach(char => {
                this.addLog(`Характеристика: ${char.uuid} (read: ${char.properties.read}, notify: ${char.properties.notify})`, 'info');
            });

            // Ищем характеристику с уведомлениями
            const notifyChar = characteristics.find(c => c.properties.notify);
            if (notifyChar) {
                this.addLog(`Найдена характеристика с уведомлениями: ${notifyChar.uuid}`, 'success');
                return notifyChar;
            }
            ...
            if (readChar) {
                this.addLog(`Найдена характеристика с чтением: ${readChar.uuid}`, 'success');
                return readChar;
            }

            // Берем первую характеристику
            if (characteristics.length > 0) {
                this.addLog(`Используем первую характеристику: ${characteristics[0].uuid}`, 'warning');
                return characteristics[0];
            }

            return null;
        } catch (error) {
            this.addLog(`Ошибка поиска характеристик: ${error.message}`, 'error');
            return null;
        }
    }
    ...
    handleData(event) {
        try {
            ...
            if (!data || data === this.lastUID || data.includes('ESP32-RFID')) {
                return;
            }
